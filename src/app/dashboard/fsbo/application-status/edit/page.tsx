...
export default function FSBOEditApplication() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      // Verify user is FSBO and needs correction
      ...
      if (profileData.approval_status !== 'needs_correction') {
        router.push('/dashboard/fsbo/application-status')
        return
      }
      ...
      <div className="max-w-xl mx-auto">
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-primary to-primary/80 px-6 py-6 text-white">
            <Link href="/dashboard/fsbo/application-status" className="text-white/80 hover:text-white text-sm mb-2 inline-block">
              ← Back to Status
            </Link>
            <h1 className="text-2xl font-bold">Edit & Resubmit Application</h1>
            <p className="text-white/80 text-sm mt-1">FSBO Seller Account</p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-4">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full bg-primary text-white px-6 py-3 rounded-lg hover:bg-primary/90 font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
