...
export default function PropertyDetailClient({ propertyId, initialData }: PropertyDetailClientProps) {
  ...
  // Sticky WhatsApp bar — refs and visibility state
  ...
  useEffect(() => {
    ...
    const update = () => setShowStickyWhatsApp(pastGallery && !contactVisible)

    // Observer 1: show bar once gallery scrolls out of view
    const galleryObserver = new IntersectionObserver(
      ([entry]) => {
        pastGallery = !entry.isIntersecting
        update()
      },
      { threshold: 0 },
    )

    // Observer 2: hide bar when contact section enters viewport
  ...
  // Sticky bar WhatsApp handler — reuses existing logic for all listing types
  const handleStickyWhatsApp = useCallback(() => {
    if (!property) return

    let phoneNumber: string | undefined | null
    let recipientName: string | undefined

    if (property.is_private_listing) {
      phoneNumber = property.owner_contact?.phone || property.owner_whatsapp
      recipientName = property.owner_contact?.name
    } else if (property.agent_profile) {
      phoneNumber = property.agent_profile.phone
      recipientName = `${property.agent_profile.first_name} ${property.agent_profile.last_name}`
    } else {
    ...
    const isUnderContract = property.status === 'under_contract' || property.status === 'pending'
    const listingType = property.listing_type || 'sale'
    const priceStr = property.price ? `$${property.price.toLocaleString()}${listingType === 'rent' ? '/month' : ''}` : ''

    const msgBuilder = isUnderContract ? buildBackupOfferMessage : buildPropertyMessage
    const message = msgBuilder({
      recipientName,
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        ...
        {displayImages.length > 0 ? (
          <div ref={galleryRef} className="mb-8 bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      {property.status !== 'sold' && (
        <button
          ...
          className={`fixed bottom-0 inset-x-0 z-50 md:hidden flex items-center justify-center gap-2 bg-[#059669] text-white font-medium h-14 transition-transform duration-200 ease-out ${
            showStickyWhatsApp ? 'translate-y-0' : 'translate-y-full'
          }`}
